...
No dia 18 de cada mês: quanto falta faturar para fechar o mês corrente com superávit.
No dia 1º de cada mês: a posição consolidada do resultado do mês anterior (fechamento).
Fontes de dados
...
Pedir ao Ricardo o extrato do ASAAS do período em aberto, se ainda não tiver sido enviado.
Classificar cada lançamento do extrato ASAAS em: receita de cliente / taxa Asaas (ignorar por enquanto — só lança no fechamento do mês seguinte) / transferência Pix interna (ignorar) / outro (perguntar).
Montar a lista `cliente → valor` da receita do período.
Mapear cada cliente do extrato para a linha correta da aba RECEITAS (ver seção "Como mapear clientes" abaixo — muitos nomes do Asaas não batem literalmente com o nome na planilha).
Lançar os valores na aba RECEITAS, na coluna "Realizado" do mês corrente (ver "Layout das colunas" abaixo).
Conferir a planilha de bancos e caixa (SICREDI + CAIXA) no mesmo período, procurando:
...
Regras já validadas para lançamentos recorrentes específicos (não perguntar de novo, a menos que o Ricardo avise que mudou):
...
Gerar a leitura correspondente à data de execução (ver seção "Saída esperada").
